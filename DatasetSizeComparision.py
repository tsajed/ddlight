...
def run_experiment(target, architecture, train_size):
    config = EasyDict({
        'Project_ID': 'Manuscript_datasize_comp',
        'arch': architecture,
        'oversampling': True,
        'undersampling': False,
        'it0_dock_cutoff': -10,
    ...
    print(f"Running experiment for target: {target}, architecture: {architecture}, train_size: {train_size}")
    print(config)
    ...
    molecule_df = molecule_df.sample(frac=1, random_state=42).reset_index(drop=True)

    if config.get('val_set',None) =='fixed':
        # Sample train data
        train_df = molecule_df.iloc[:config.end]
        # Sample validation data (same 500K mols for each expt) from remaining molecules
        val_df = molecule_df.iloc[config.end:config.end + 500000]
    else:
        train_df, val_df = train_test_split(molecule_df, test_size=min(len(molecule_df)//2,500000) / len(molecule_df), random_state=42)
    ...
    if architecture in ('gcn', 'gat', 'gin', 'gine'):
        train_labels = [1 if value < config.it0_dock_cutoff else 0 for value in it0_data_train_dock_scores[:config.end]]
        val_labels = [1 if value < config.it0_dock_cutoff else 0 for value in it0_data_val_dock_scores[:500000]]
        ...
        if config.oversampling:
            ...
            train_smiles_resampled, train_labels_resampled = ros.fit_resample(
                np.array(it0_data_train_smiles_list[:config.end]).reshape(-1, 1),
                train_labels
            )
            ...
            train_dataset = MyDataset(
                it0_data_train_mol_ids[:config.end],
                train_smiles_resampled,
                train_labels_resampled
            )
        else:
            train_dataset = MyDataset(it0_data_train_mol_ids[:config.end], it0_data_train_smiles_list[:config.end], train_labels)

        val_dataset = MyDataset(it0_data_val_mol_ids[:500000], it0_data_val_smiles_list[:500000], val_labels)
        ...
        train_graph_model(model, train_loader, val_loader, num_epochs=40, config=config, patience=10,
                          model_save_path='/groups/cherkasvgrp/Student_backup/mkpandey/My_Projects/DDSgroups/VinaAL/Experiments/Datasetcomp/')
        ...
        if config.oversampling:
            ros = RandomOverSampler(random_state=42)
            X_train_resampled, y_train_resampled = ros.fit_resample(it0_train_features[:config.end], train_labels)
            print('Resampled train_labels distribution:', Counter(y_train_resampled))
        else:
            X_train_resampled, y_train_resampled = it0_train_features[:config.end], train_labels
        ...
        val_dataset = TensorDataset(torch.tensor(it0_val_features[:500000], dtype=torch.float32), torch.tensor(val_labels, dtype=torch.long))
        ...
        if config.oversampling:
            ros = RandomOverSampler(random_state=42)
            train_smiles_resampled, train_labels_resampled = ros.fit_resample(np.array(it0_data_train_smiles_list[:config.end]).reshape(-1, 1), train_labels)
            train_smiles_resampled = train_smiles_resampled.flatten().tolist()
        else:
            train_smiles_resampled, train_labels_resampled = it0_data_train_smiles_list[:config.end], train_labels

        initialX = model.tokenizer(train_smiles_resampled, padding=True, truncation=True, return_tensors="pt")
        valX = model.tokenizer(it0_data_val_smiles_list[:500000], padding=True, truncation=True, return_tensors="pt")
        ...
        if config.oversampling:
            ...
            X_train_resampled, y_train_resampled = ros.fit_resample(
                it0_train_features[:config.end], train_labels
            )
            print('Resampled train_labels distribution:', Counter(y_train_resampled))
        else:
        ...
        val_features_limited = it0_val_features[:500000]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_size', type=int, required=True, help="Training dataset size")
    ...
    # Call the function with specified target, architecture, and train size
    run_experiment(target, architecture, train_size)
