...
def run_inference_scatter(model, iteration_dir, split_molecule_df, used_zinc_ids, config, tokenizer=None, task_id=0, num_tasks=1):
    ...
    # for mol_id, smiles, label, pred_proba, score in tqdm(zip(mol_ids, smiles_list, pred_labels, pred_proba, scores)):
    #     # Handle virtual hits (label == 1)
    #     if label == 1:
    #         virtual_hits.append((mol_id, smiles, pred_proba))
    #     else:
    #         if (config.al_params.drop_db) and (config.al_params.acquisition_function in ('greedy', 'random')): # drop predicted inactives by considering them used, so that they dont participate in next al iteration.
    #             used_zinc_ids.add(mol_id)
    #         elif config.al_params.drop_db and config.al_params.acquisition_function in ('unc','ucb','bald','margin','entropy','least_confidence'):
    #             low_uncertainty_threshold = min(low_uncertainty_threshold,np.quantile(uncertainty_scores, 0.10))# 0.25)
    #             # print('low_uncertainity_thresh ', low_uncertainty_threshold)
    #             # print('uncertainity scores ', sorted(uncertainty_scores))
    #             if score< low_uncertainty_threshold: # drop most certain inactive prediction mols by considering them used, so that they dont participate in next al iteration.
    #                 used_zinc_ids.add(mol_id) 
    ...
    #     all_molecules.append(((score, mol_id, smiles, label)))
...
#                               rank, world_size, smiles_2_dockscore_gt=None, tokenizer = None):
...
#                     split_molecule_df = molecule_df.iloc[start_idx:end_idx]  # Process only assigned chunk
...
#             else:
#                 inf_res, used_zinc_ids = run_inference(model, molecule_df, batch_size, top_k//1, used_zinc_ids, config, tokenizer=tokenizer) # acquire topk, then topk/2, then topk/3,...., to not change the training distribution too much from apriori (real-world) distribution
